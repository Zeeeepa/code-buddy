...
│            ▼                                                            │
│   ┌────────────────────┐                                               │
│   │  1️⃣  RETRIEVER     │ ◄── Cherche dans la base de connaissances    │
│   │   (recherche)      │                                               │
│   └─────────┬──────────┘                                               │
│             │                                                           │
│             ▼                                                           │
│   ┌────────────────────┐                                               │
│   │  📄 Documents      │  (Top K les plus pertinents)                  │
│   │    trouvés         │                                               │
│   └─────────┬──────────┘                                               │
│             │                                                           │
│             ▼                                                           │
│   ┌────────────────────┐                                               │
│   │  2️⃣  AUGMENTATION  │ ◄── Injecte dans le prompt                   │
│   │   (enrichit)       │                                               │
│   └─────────┬──────────┘                                               │
│             │                                                           │
│             ▼                                                           │
│   ┌────────────────────┐                                               │
│   │  3️⃣  GENERATION    │ ◄── LLM génère avec le contexte              │
│   │   (répond)         │                                               │
│   └─────────┬──────────┘                                               │
│             │                                                           │
│             ▼                                                           │
...
│                                                                         │
│  Embedding (simplifié, réellement 384+ dimensions) :                    │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  [0.023, -0.156, 0.089, 0.234, -0.067, 0.145, ..., 0.012]      │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  ✨ Propriétés magiques :                                               │
│  ┌────────────────────────┬────────────────────────────────────────┐   │
...
│  │ Même sens, mots diff.  │ → Vecteurs similaires !                │   │
│  └────────────────────────┴────────────────────────────────────────┘   │
│                                                                         │
│  Exemple de proximité :                                                 │
│                                                                         │
│  "calculateTotal"     ←──── très proche ────→  "computeSum"            │
│  "calculateTotal"     ←──── proche ──────────→ "sumPrices"             │
│  "calculateTotal"     ←──── éloigné ─────────→ "sendEmail"             │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│   │                       │      │                       │             │
│   │  1. fetchUser    0.89 │      │  1. getUserById  9.2  │             │
│   │  2. getUser      0.87 │      │  2. getUser      7.1  │             │
│   │  3. findUser     0.82 │      │  3. userById     5.3  │             │
│   │  4. loadUser     0.79 │      │  4. fetchUser    4.8  │             │
│   └───────────┬───────────┘      └───────────┬───────────┘             │
│               │                               │                         │
│               └───────────┬───────────────────┘                        │
│                           ▼                                             │
...
│               │  Score = Σ 1/(k+rank) │                                 │
│               │                       │                                 │
│               │  1. getUserById 0.024 │  ← Monte grâce aux keywords    │
│               │  2. getUser     0.023 │                                 │
│               │  3. fetchUser   0.022 │                                 │
│               │  4. findUser    0.018 │                                 │
│               └───────────────────────┘                                 │
