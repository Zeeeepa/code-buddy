# 🔍 Index

> Index alphabétique des concepts, techniques et outils

---

## A

- **Agent** — [Ch.02](02-role-des-agents.md), [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)
- **AI! / AI? (commentaires)** — [Ch.18](18-productivite-cli.md)
- **AI Code Review** — [Ch.06](06-repair-reflexion.md), [Ch.18](18-productivite-cli.md)
- **Agent multi-agent** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)
- **Agent spécialisé** — [Ch.15](15-architecture-complete.md)
- **Agentic loop** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)
- **Algorithme UCB1** — [Ch.05](05-mcts.md)
- **Analyse de dépendances** — [Ch.08](08-dependency-aware-rag.md)
- **Apprentissage** — [Ch.14](14-apprentissage-persistant.md)
- **Apprentissage épisodique** — [Ch.14](14-apprentissage-persistant.md)
- **Apprentissage few-shot** — [Ch.01](01-comprendre-les-llms.md)
- **Apprentissage in-context** — [Ch.01](01-comprendre-les-llms.md)
- **Apprentissage persistant** — [Ch.14](14-apprentissage-persistant.md)
- **Approval modes** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)
- **Architecture Transformer** — [Ch.01](01-comprendre-les-llms.md)
- **AST (Abstract Syntax Tree)** — [Ch.07](07-rag-moderne.md), [Ch.08](08-dependency-aware-rag.md)
- **Assistant** — [Ch.02](02-role-des-agents.md)
- **Attention (mécanisme)** — [Ch.01](01-comprendre-les-llms.md)
- **Auto-lint** — [Ch.06](06-repair-reflexion.md), [Ch.18](18-productivite-cli.md)
- **Auto-repair** — [Ch.06](06-repair-reflexion.md)
- **Auto-test** — [Ch.06](06-repair-reflexion.md), [Ch.18](18-productivite-cli.md)
- **Autorégressif** — [Ch.01](01-comprendre-les-llms.md)

## B

- **Backpropagation (MCTS)** — [Ch.05](05-mcts.md)
- **Bash (outil)** — [Ch.10](10-tool-use.md)
- **Beam search** — [Ch.04](04-tree-of-thought.md)
- **BFS (Breadth-First Search)** — [Ch.04](04-tree-of-thought.md)
- **BPE (Byte Pair Encoding)** — [Ch.01](01-comprendre-les-llms.md)
- **Budget de tokens** — [Ch.09](09-context-compression.md)

## C

- **Cache sémantique** — [Ch.12](12-optimisations-cognitives.md)
- **Cache d'outils** — [Ch.12](12-optimisations-cognitives.md)
- **CI/CD Integration** — [Ch.13](13-optimisations-systeme.md), [Ch.18](18-productivite-cli.md)
- **CircleCI** — [Ch.13](13-optimisations-systeme.md)
- **Context files** — [Ch.18](18-productivite-cli.md)
- **Compress (commande)** — [Ch.18](18-productivite-cli.md)
- **Chain-of-Thought (CoT)** — [Ch.04](04-tree-of-thought.md)
- **Chatbot** — [Ch.02](02-role-des-agents.md)
- **ChatRepair** — [Ch.06](06-repair-reflexion.md)
- **Chunking** — [Ch.07](07-rag-moderne.md)
- **Chunking AST** — [Ch.07](07-rag-moderne.md)
- **Chunking sémantique** — [Ch.07](07-rag-moderne.md)
- **Claude** — [Ch.01](01-comprendre-les-llms.md), [Ch.11](11-plugins-mcp.md)
- **Code intelligence** — [Ch.10](10-tool-use.md)
- **Code Review (AI)** — [Ch.06](06-repair-reflexion.md), [Ch.18](18-productivite-cli.md)
- **CodeRAG** — [Ch.08](08-dependency-aware-rag.md)
- **Complétion de code** — [Ch.02](02-role-des-agents.md)
- **Compression de contexte** — [Ch.09](09-context-compression.md)
- **Confirmation (dialogue)** — [Ch.03](03-anatomie-agent.md), [Ch.10](10-tool-use.md)
- **Consolidation mémoire** — [Ch.14](14-apprentissage-persistant.md)
- **Contexte (fenêtre)** — [Ch.01](01-comprendre-les-llms.md), [Ch.09](09-context-compression.md)
- **Cosine similarity** — [Ch.07](07-rag-moderne.md), [Ch.12](12-optimisations-cognitives.md)
- **Coût API** — [Ch.12](12-optimisations-cognitives.md), [Ch.13](13-optimisations-systeme.md)
- **Cross-encoder** — [Ch.07](07-rag-moderne.md)

## D

- **Déduplication sémantique** — [Ch.09](09-context-compression.md)
- **Delegate (commande)** — [Ch.18](18-productivite-cli.md)
- **Dependency-Aware RAG** — [Ch.08](08-dependency-aware-rag.md)
- **DFS (Depth-First Search)** — [Ch.04](04-tree-of-thought.md)
- **Diagramme de dépendances** — [Ch.08](08-dependency-aware-rag.md)

## E

- **Early stopping** — [Ch.04](04-tree-of-thought.md), [Ch.05](05-mcts.md)
- **Embeddings** — [Ch.07](07-rag-moderne.md)
- **Espace vectoriel** — [Ch.07](07-rag-moderne.md)
- **Évaluation (ToT)** — [Ch.04](04-tree-of-thought.md)
- **Exécution parallèle** — [Ch.10](10-tool-use.md), [Ch.13](13-optimisations-systeme.md)
- **Expansion (MCTS)** — [Ch.05](05-mcts.md)

## F

- **FAISS** — [Ch.07](07-rag-moderne.md)
- **Feedback de tests** — [Ch.06](06-repair-reflexion.md)
- **Few-shot learning** — [Ch.01](01-comprendre-les-llms.md)
- **File operations** — [Ch.10](10-tool-use.md)
- **Fine-tuning** — [Ch.01](01-comprendre-les-llms.md)
- **Flow state** — [Ch.13](13-optimisations-systeme.md)
- **FrugalGPT** — [Ch.13](13-optimisations-systeme.md)
- **Function calling** — [Ch.10](10-tool-use.md)

## G

- **Génération autorégressive** — [Ch.01](01-comprendre-les-llms.md)
- **GitHub Actions** — [Ch.13](13-optimisations-systeme.md), [Ch.18](18-productivite-cli.md)
- **GitLab CI** — [Ch.13](13-optimisations-systeme.md)
- **Glob (outil)** — [Ch.10](10-tool-use.md)
- **GPT** — [Ch.01](01-comprendre-les-llms.md)
- **Graphe de dépendances** — [Ch.08](08-dependency-aware-rag.md)
- **Grep (outil)** — [Ch.10](10-tool-use.md)
- **Grok** — [Ch.01](01-comprendre-les-llms.md), [Ch.15](15-architecture-complete.md)
- **Code Buddy** — [Ch.00](00-avant-propos.md), [Ch.15](15-architecture-complete.md)
- **Guardrails** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)

## H

- **Hallucination** — [Ch.01](01-comprendre-les-llms.md), [Ch.07](07-rag-moderne.md)
- **Hooks** — [Ch.11](11-plugins-mcp.md)
- **Hooks lifecycle** — [Ch.11](11-plugins-mcp.md), [Ch.18](18-productivite-cli.md)
- **Hybrid search** — [Ch.07](07-rag-moderne.md)

## I

- **Image input** — [Ch.10](10-tool-use.md), [Ch.18](18-productivite-cli.md)
- **Import graph** — [Ch.08](08-dependency-aware-rag.md)
- **In-context learning** — [Ch.01](01-comprendre-les-llms.md)
- **Index vectoriel** — [Ch.07](07-rag-moderne.md)
- **Iterative repair** — [Ch.06](06-repair-reflexion.md)

## J

- **JetBrains Research** — [Ch.09](09-context-compression.md)
- **JSON Schema** — [Ch.10](10-tool-use.md)

## K

- **K-nearest neighbors** — [Ch.07](07-rag-moderne.md)

## L

- **Latence** — [Ch.13](13-optimisations-systeme.md)
- **Lazy loading** — [Ch.13](13-optimisations-systeme.md)
- **LLM (Large Language Model)** — [Ch.01](01-comprendre-les-llms.md)
- **LLMCompiler** — [Ch.13](13-optimisations-systeme.md)
- **Localisation de fautes** — [Ch.06](06-repair-reflexion.md)
- **LRU Cache** — [Ch.12](12-optimisations-cognitives.md)

## M

- **MCP (Model Context Protocol)** — [Ch.11](11-plugins-mcp.md)
- **MCTS (Monte-Carlo Tree Search)** — [Ch.05](05-mcts.md)
- **Megathink** — [Ch.04](04-tree-of-thought.md)
- **Mémoire** — [Ch.03](03-anatomie-agent.md), [Ch.14](14-apprentissage-persistant.md)
- **Mémoire épisodique** — [Ch.14](14-apprentissage-persistant.md)
- **Mémoire procédurale** — [Ch.14](14-apprentissage-persistant.md)
- **Mémoire prospective** — [Ch.14](14-apprentissage-persistant.md)
- **Mémoire sémantique** — [Ch.14](14-apprentissage-persistant.md)
- **MemGPT** — [Ch.14](14-apprentissage-persistant.md)
- **Model routing** — [Ch.13](13-optimisations-systeme.md)
- **Multi-agent** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)
- **Multi-edit** — [Ch.10](10-tool-use.md)
- **Multi-head attention** — [Ch.01](01-comprendre-les-llms.md)

## N

- **Next token prediction** — [Ch.01](01-comprendre-les-llms.md)
- **NLP** — [Ch.01](01-comprendre-les-llms.md)

## O

- **Observation masking** — [Ch.09](09-context-compression.md)
- **OpenAI** — [Ch.01](01-comprendre-les-llms.md), [Ch.10](10-tool-use.md)
- **Optimisation** — [Ch.12](12-optimisations-cognitives.md), [Ch.13](13-optimisations-systeme.md)
- **Orchestrateur** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md)
- **Outils (tools)** — [Ch.10](10-tool-use.md)

## P

- **Parallélisation** — [Ch.10](10-tool-use.md), [Ch.13](13-optimisations-systeme.md)
- **Patch (réparation)** — [Ch.06](06-repair-reflexion.md)
- **Permission** — [Ch.03](03-anatomie-agent.md), [Ch.10](10-tool-use.md)
- **Personas** — [Ch.18](18-productivite-cli.md)
- **Pipeline RAG** — [Ch.07](07-rag-moderne.md)
- **Plugin** — [Ch.11](11-plugins-mcp.md)
- **Positional encoding** — [Ch.01](01-comprendre-les-llms.md)
- **Pre-training** — [Ch.01](01-comprendre-les-llms.md)
- **Priorité (contexte)** — [Ch.09](09-context-compression.md)
- **Prompt caching** — [Ch.12](12-optimisations-cognitives.md), [Ch.13](13-optimisations-systeme.md)
- **Prompt engineering** — [Ch.01](01-comprendre-les-llms.md)
- **Prompt externe (Markdown)** — [Ch.16](16-system-prompts-securite.md)
- **Prompt hardening** — [Ch.16](16-system-prompts-securite.md)
- **Prompt injection** — [Ch.16](16-system-prompts-securite.md)

## Q

- **Query expansion** — [Ch.07](07-rag-moderne.md)

## R

- **RAG (Retrieval-Augmented Generation)** — [Ch.07](07-rag-moderne.md), [Ch.08](08-dependency-aware-rag.md)
- **Raisonnement** — [Ch.03](03-anatomie-agent.md), [Ch.04](04-tree-of-thought.md)
- **ReAct (pattern)** — [Ch.03](03-anatomie-agent.md)
- **Recherche hybride** — [Ch.07](07-rag-moderne.md)
- **Recherche sémantique** — [Ch.07](07-rag-moderne.md)
- **Réflexion** — [Ch.06](06-repair-reflexion.md)
- **Réparation itérative** — [Ch.06](06-repair-reflexion.md)
- **Reranking** — [Ch.07](07-rag-moderne.md)
- **RethinkMCTS** — [Ch.05](05-mcts.md)
- **Ripgrep** — [Ch.10](10-tool-use.md)
- **RNN** — [Ch.01](01-comprendre-les-llms.md)
- **Rollout (MCTS)** — [Ch.05](05-mcts.md)
- **Routage de modèles** — [Ch.13](13-optimisations-systeme.md)

## S

- **Sandbox** — [Ch.03](03-anatomie-agent.md), [Ch.10](10-tool-use.md)
- **SBFL (Spectrum-Based Fault Localization)** — [Ch.06](06-repair-reflexion.md)
- **Sécurité** — [Ch.03](03-anatomie-agent.md), [Ch.15](15-architecture-complete.md), [Ch.16](16-system-prompts-securite.md)
- **Sécurité CLI** — [Ch.16](16-system-prompts-securite.md)
- **Self-attention** — [Ch.01](01-comprendre-les-llms.md)
- **Sélection (MCTS)** — [Ch.05](05-mcts.md)
- **Shell prefix (!)** — [Ch.18](18-productivite-cli.md)
- **Simulation (MCTS)** — [Ch.05](05-mcts.md)
- **Skills** — [Ch.11](11-plugins-mcp.md), [Ch.18](18-productivite-cli.md)
- **Slash commands** — [Ch.11](11-plugins-mcp.md), [Ch.18](18-productivite-cli.md)
- **Streaming** — [Ch.10](10-tool-use.md)
- **Symbol search** — [Ch.10](10-tool-use.md)
- **System prompt** — [Ch.16](16-system-prompts-securite.md)

## T

- **Taxonomie des agents** — [Ch.02](02-role-des-agents.md)
- **TDD (Test-Driven Development)** — [Ch.06](06-repair-reflexion.md), [Ch.18](18-productivite-cli.md)
- **Temperature** — [Ch.01](01-comprendre-les-llms.md)
- **Test feedback** — [Ch.06](06-repair-reflexion.md)
- **Think (keyword)** — [Ch.04](04-tree-of-thought.md)
- **Token** — [Ch.01](01-comprendre-les-llms.md)
- **Tokenization** — [Ch.01](01-comprendre-les-llms.md)
- **Tool-use** — [Ch.10](10-tool-use.md)
- **Top-k** — [Ch.01](01-comprendre-les-llms.md)
- **Top-p (nucleus)** — [Ch.01](01-comprendre-les-llms.md)
- **ToT (Tree-of-Thought)** — [Ch.04](04-tree-of-thought.md)
- **Transformer** — [Ch.01](01-comprendre-les-llms.md)
- **Transport (MCP)** — [Ch.11](11-plugins-mcp.md)
- **Tree search** — [Ch.04](04-tree-of-thought.md), [Ch.05](05-mcts.md)

## U

- **UCB1 (Upper Confidence Bound)** — [Ch.05](05-mcts.md)
- **Ultrathink** — [Ch.04](04-tree-of-thought.md)

## V

- **Validation (outils)** — [Ch.10](10-tool-use.md)
- **Vector database** — [Ch.07](07-rag-moderne.md)
- **Vision (image input)** — [Ch.10](10-tool-use.md), [Ch.18](18-productivite-cli.md)
- **Vocabulaire (tokenizer)** — [Ch.01](01-comprendre-les-llms.md)
- **Voice input** — [Ch.10](10-tool-use.md), [Ch.18](18-productivite-cli.md)

## W

- **Watch mode** — [Ch.18](18-productivite-cli.md)
- **Whisper (transcription)** — [Ch.18](18-productivite-cli.md)
- **Window (contexte)** — [Ch.01](01-comprendre-les-llms.md), [Ch.09](09-context-compression.md)
- **Workflow** — [Ch.14](14-apprentissage-persistant.md)

## X

- **xAI** — [Ch.01](01-comprendre-les-llms.md), [Ch.15](15-architecture-complete.md)

## Z

- **Zero-shot** — [Ch.01](01-comprendre-les-llms.md)

---

| 📖 Retour au sommaire |
|:---------------------:|
| [README](README.md) |
