/**
 * Sync Bindings for Unified Scripting
 *
 * Provides scripting functions for cross-session synchronization:
 ...
 *
 * Migrated from src/fcs/sync-bindings.ts
 */

import { CodeBuddyValue, CodeBuddyFunction, CodeBuddyScriptConfig } from './types.js';
import {
  SyncManager,
  SyncState,
  SyncConflict,
...
export interface SyncBindingsConfig extends Partial<CodeBuddyScriptConfig> {
  sessionId?: string;
  nodeId?: string;
...
export class WorkspaceStateTracker {
  ...
  trackFile(filePath: string, content: string, isOpen = false): void {
  ...
  diffWith(other: WorkspaceSnapshot): FileDiff[] {
    ...
    for (const p of allPaths) {
      ...
      const snapshot = other.files.get(p);

      if (current && !snapshot) {
        diffs.push({ path: p, type: 'added', local: current.content });
      } else if (!current && snapshot) {
        diffs.push({ path: p, type: 'deleted', remote: snapshot.content });
...
/**
 * Create sync bindings for the scripting runtime
 */
export async function createSyncBindings(
  ...
  const sync: Record<string, CodeBuddyFunction | CodeBuddyValue> = {};
  ...
  sync.list = (): string => {
    ...
    const lines = snapshots.map(s => {
      ...
      const snapName = s.metadata.name || 'Unnamed';
      return `${s.id}: ${snapName} (${date}) - ${s.files.size} files`;
    });
