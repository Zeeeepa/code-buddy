  ...
  async autoFix(failure: CIFailure): Promise<AutoFixResult> {
    ...
    for (let attempt = previousAttempts + 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (fixAttempt.localTestsPassed) {
        ...
        try {
          const branchName = `autofix/${failure.branch}-${failure.runId.substring(0, 8)}`;
          await this.bash.execute(`git checkout -b ${branchName}`);
          await this.bash.execute('git add -A');
          await this.bash.execute(`git commit -m "fix(ci): auto-fix ${failure.jobName} failure

Root cause: ${fixAttempt.rootCause.substring(0, 200)}

Auto-generated by Code Buddy CI AutoFix Pipeline.
Attempt ${attempt}/${MAX_ATTEMPTS}."`);

          const pushResult = await this.bash.execute(`git push -u origin ${branchName}`);
          if (pushResult.success) {
            fixAttempt.pushed = true;

            // Try to create PR via gh CLI
            const prResult = await this.bash.execute(
              `gh pr create --title "fix(ci): auto-fix ${failure.jobName}" --body "Auto-fix for CI failure in ${failure.jobName}.\\n\\n**Root cause:** ${fixAttempt.rootCause.substring(0, 300).replace(/"/g, '\\"')}\\n\\n**Files:** ${fixAttempt.filesModified.join(', ')}\\n\\nGenerated by Code Buddy CI AutoFix Pipeline." --base ${failure.branch}`
            );
            if (prResult.success && prResult.output) {
              result.prUrl = prResult.output.trim();
            }
          }

          // Switch back to original branch
          await this.bash.execute(`git checkout ${failure.branch}`);
        } catch (err) {
